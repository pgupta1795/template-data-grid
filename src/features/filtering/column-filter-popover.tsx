...
import type { ColumnMeta, ColumnType, SelectOption } from "@/types/column-types"
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
    ...
    <Popover>
      ...
      >
        {isFiltered ? (
          ...
          <ListFilter className="h-3.5 w-3.5 text-muted-foreground/40 opacity-0 group-hover/header:opacity-100 transition-opacity" />
        )}
      </PopoverTrigger>
      <PopoverContent
...
function StringFilter({ column }: { column: Column<GridRow, unknown> }) {
  const current = (column.getFilterValue() as StringFilterValue | undefined) ?? {
    value: "",
  ...
  return (
    <div className="flex flex-col gap-1 max-h-48 overflow-y-auto">
      {sortedTags.map(([tag, count]) => (
        <label
          key={tag}
          className="flex items-center gap-2 cursor-pointer hover:bg-muted/40 px-1 py-0.5 rounded text-xs"
        >
          <Checkbox
...
function SelectTypeFilter({
  ...
  const items = React.useMemo(() => {
    ...
    return fromFacet.map((f) => ({ value: f.value, label: f.value, count: f.count }))
  }, [facetMap, options])
...
function BooleanFilter({ column }: { column: Column<GridRow, unknown> }) {
  ...
  const current: BoolVal = raw === "true" ? "true" : raw === "false" ? "false" : "all"
